            ...
            <section class="flex flex-col break-words bg-white sm:border-1 sm:rounded-md sm:shadow-sm sm:shadow-lg">
                ...
                <form class="w-full px-6 space-y-6 sm:px-10 sm:space-y-8" method="POST" action="{{ route('password.email') }}">
                    ...
                    <div class="flex flex-wrap items-center justify-center pb-6 space-y-6 sm:pb-10 sm:space-y-0 sm:justify-between">
                        <button type="submit"
                        class="w-full p-3 text-base font-bold leading-normal text-gray-100 no-underline whitespace-no-wrap bg-green-500 rounded-lg select-none hover:bg-green-700 sm:w-auto sm:px-4 sm:order-1">
                            {{ __('Send Password Reset Link') }}
                        </button>

                        <p class="mt-4 text-xs text-green-500 no-underline whitespace-no-wrap hover:text-green-700 hover:underline sm:text-sm sm:order-0 sm:m-0">
                            <a class="text-green-500 no-underline hover:text-green-700" href="{{ route('login') }}">
                                {{ __('Back to login') }}
                            </a>
                        </p>
                    </div>
